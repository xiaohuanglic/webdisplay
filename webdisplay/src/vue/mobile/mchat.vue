...
<script>
import chatmodel from './mchatmodel.vue'
...
const source = CancelToken.source();
  var addimage=Vue.extend({
            props:['imgsrc'],
            template:'<img class="mgetpicmiddle" :src=this.imgsrc>',
            ...
            var that = this
              ...
              //判断需要有没有输入数据
                keyevent($event){
                  ...
                  var currentimg = document.getElementsByClassName("mgetpicmiddle");
                  console.log()
//                  component: (...)
// imgsrc: (...)
                  var getnewlist =[];
                                ...
                                for(var j = 0;j<getdata[i]["imgpath"].length;j++){
                                    if(getdata[i]["imgpath"][j]!=""){
                                         ...
                                         var bigImg = document.createElement("img");
                                          bigImg.setAttribute("class", "mshowchatpic chatpadding");
...
#sendmsg #getsendmsg img.mgetpicmiddle{
    width: 50% ;
    margin-left: 2%;
}
.getpicsmall{
